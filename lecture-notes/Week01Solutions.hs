...
-- This works by reversing the input so that the spaces to be dropped
-- are at the start, using 'dropSpaces' to remove those spaces, then
-- reversing again to put the letters back in the right order.

dropTrailingSpaces :: [Char] -> [Char]
dropTrailingSpaces xs = reverse (dropSpaces (reverse xs))

{- Alternative, which names the intermediate steps:

dropTrailingSpaces xs = answer
  where reversed = reverse xs
        dropped  = dropSpaces reversed
        answer   = reverse dropped

Alternatively, using knowledge from Week 03 to create a pipeline (read
from right to left!)
...
-- You could also do this using guards, or lots of if-then-else
